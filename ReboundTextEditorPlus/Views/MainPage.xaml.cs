...
public sealed partial class MainPage : Page
{
    ...
    public MainPage()
    {
        ...
        //Document config
        editor.Document.Selection.CharacterFormat.Size = (float)10.5;
    ...
    private async void SB2_Click(object sender, RoutedEventArgs e)
    {
        ...
        //            editor.Document.SaveToStream(TextGetOptions.FormatRtf, RAS);
    ...
    private async void MenuFlyoutItem_Click_24(object sender, RoutedEventArgs e)
    {
        ...
        //    {
        //        IRandomAccessStream RAS = await TXTFile.OpenAsync(FileAccessMode.ReadWrite);
        //        editor.Document.SaveToStream(TextGetOptions.None, RAS);
    ...
    public async Task EraseFile()
    {
        ...
        if (TemplateBlankClicked != true)
        {
            ...
            editor.Document.LoadFromStream(TextSetOptions.FormatRtf, RAStream);
        }
        else
        {
    ...
    {
        //if (GetText(editor) == GetText(RTB) || GetText(editor) == string.Empty)
        //{
        ...
        //else
        //{
        //    if (GetText(editor) == GetText(RTB) && TXTFile != null && ((Window.Current.Content as Frame).Content as MainPage).TabbedView.SelectedItem != null)
        //    {
        ...
        //    }
        //    if (GetText(editor) != GetText(RTB) || TXTFile == null)
        //    {
        //        FileNotSaved.Visibility = Visibility.Visible;
        //        isWorkSaved = false;
        //    }
    ...
    public async Task SaveFile(bool Background, bool Close, bool Erase, bool NoFile, bool triggeredByAutosave = false)
    {
        ...
        //            editor.Document.SetText(Options, string.Empty);
    ...
    public async Task Open()
    {
        ...
        //        {
        //            IsRTF = true;
        //            editor.Document.LoadFromStream(TextSetOptions.FormatRtf, RAS);
        //            RTB.Document.LoadFromStream(TextSetOptions.FormatRtf, RAS);
    ...
    private void Undo_Click(object Sender, RoutedEventArgs EvArgs) => editor.Document.Undo();

    private void Redo_Click(object Sender, RoutedEventArgs EvArgs) => editor.Document.Redo();
    ...
    {
        editor.Document.Selection.Paste(1);
    }
    ...
    {
        editor.Document.Selection?.ChangeCase(LetterCase.Lower);
    }

    private void MenuFlyoutItem_Click_26(object sender, RoutedEventArgs e)
    {
        editor.Document.Selection?.ChangeCase(LetterCase.Upper);
    }
    ...
    {
        editor.Document.Selection.Text += DateTime.Now.ToString();
        DateTimeInsert.Flyout.Hide();
    }

    private void Button_Click_6(object Sender, RoutedEventArgs EvArgs)
    {
        if (string.IsNullOrEmpty(editor.Document.Selection.Link)) return; editor.Document.Selection.Link = "";
    }

    private void Button_Click_5(object Sender, RoutedEventArgs EvArgs)
    {
        if (string.IsNullOrEmpty(HLBox.Text)) return;
        if (string.IsNullOrEmpty(editor.Document.Selection.Text)) return;
        editor.Document.Selection.Link = "\"" + HLBox.Text + "\"";
    ...
    private async void Image_Insert_Click(object Sender, RoutedEventArgs EvArgs)
    {
        ...
        //        editor.Document.Selection.InsertImage(BMP.PixelWidth, BMP.PixelHeight, 0, VerticalCharacterAlignment.Baseline, IMGFile.DisplayName, RAC);
        //    }
        //    catch (Exception Ex)
        //    {
    ...
    private async void editor_SelectionChanged(object Sender, RoutedEventArgs EvArgs)
    {
        //Variables
        ...
        if (!(ReadButton == null))
        {
            if (editor.Document.Selection.Text == "") ReadButton.IsEnabled = false;
    ...
    {
        if (editor.Document.CanCopy() == true)
        {
            CB.IsEnabled = true;
        ...
        if (editor.Document.CanPaste() == true)
        {
        ...
        if (editor.Document.CanUndo() == true) Undo.IsEnabled = true;
        else Undo.IsEnabled = false;

        if (editor.Document.CanRedo() == true) Redo.IsEnabled = true;
    ...
    {
        //LeftIndent.Value = editor.Document.Selection.ParagraphFormat.LeftIndent;
        LeftInd.Value = editor.Document.Selection.ParagraphFormat.LeftIndent;
        LeftIndent.Text = editor.Document.Selection.ParagraphFormat.LeftIndent.ToString();

        //RightIndent.Value = editor.Document.Selection.ParagraphFormat.RightIndent;
        RightInd.Value = editor.Document.Selection.ParagraphFormat.RightIndent;
        RightIndent.Text = editor.Document.Selection.ParagraphFormat.RightIndent.ToString();

        TabIndent.Value = editor.Document.Selection.ParagraphFormat.FirstLineIndent;
    ...
    private async void FindBTN_Click(object Sender, RoutedEventArgs EvArgs)
    {
        if (CaseSensBox.IsChecked == true)
        {
            Findeditor.Document.SetText(TextSetOptions.FormatRtf, GetText(editor));

            string textToFind = FindTextBox.Text;
            if (textToFind != null)
            {
                await Find(textToFind, FindOptions.Case);
                Findeditor.IsReadOnly = true;
            }
        }
        if (FullWordsBox.IsChecked == true)
        {
    ...
    private void editor_TextChanged(object Sender, RoutedEventArgs EvArgs)
    {
        CheckForSaving();
    }
    ...
    public void SCheckOn()
    {
        ...
        editor.Document.GetText(TextGetOptions.FormatRtf, out _);
        //editor.Document.SetText(Options, T);
        editor.Document.SetText(Options, string.Empty);
        editor.Document.SetText(Options, Y);
        //}
        //else
        //{
    ...
    {
        if ((sender as MenuFlyoutItem).Text == "8") editor.Document.Selection.CharacterFormat.Size = 8;
        if ((sender as MenuFlyoutItem).Text == "9") editor.Document.Selection.CharacterFormat.Size = 9;
        if ((sender as MenuFlyoutItem).Text == "10") editor.Document.Selection.CharacterFormat.Size = 10;
        if ((sender as MenuFlyoutItem).Text == "10.5" || (sender as MenuFlyoutItem).Text == "Default") editor.Document.Selection.CharacterFormat.Size = (float)10.5;
        if ((sender as MenuFlyoutItem).Text == "11") editor.Document.Selection.CharacterFormat.Size = 11;
        if ((sender as MenuFlyoutItem).Text == "12") editor.Document.Selection.CharacterFormat.Size = 12;
        if ((sender as MenuFlyoutItem).Text == "14") editor.Document.Selection.CharacterFormat.Size = 14;
        if ((sender as MenuFlyoutItem).Text == "16") editor.Document.Selection.CharacterFormat.Size = 16;
        if ((sender as MenuFlyoutItem).Text == "18") editor.Document.Selection.CharacterFormat.Size = 18;
        if ((sender as MenuFlyoutItem).Text == "20") editor.Document.Selection.CharacterFormat.Size = 20;
        if ((sender as MenuFlyoutItem).Text == "22") editor.Document.Selection.CharacterFormat.Size = 22;
        if ((sender as MenuFlyoutItem).Text == "24") editor.Document.Selection.CharacterFormat.Size = 24;
        if ((sender as MenuFlyoutItem).Text == "26") editor.Document.Selection.CharacterFormat.Size = 26;
        if ((sender as MenuFlyoutItem).Text == "28") editor.Document.Selection.CharacterFormat.Size = 28;
        if ((sender as MenuFlyoutItem).Text == "36") editor.Document.Selection.CharacterFormat.Size = 36;
        if ((sender as MenuFlyoutItem).Text == "48") editor.Document.Selection.CharacterFormat.Size = 48;
        if ((sender as MenuFlyoutItem).Text == "72") editor.Document.Selection.CharacterFormat.Size = 72;
    }
    ...
    public void CheckAlignment()
    {
        ...
        if (editor.Document.Selection.ParagraphFormat.Alignment == ParagraphAlignment.Undefined)
        {
            LeftAl.IsChecked = false;
            RightAl.IsChecked = false;
            CenterAl.IsChecked = false;
    ...
    public void CheckFormatting()
    {
        ...
        if (editor.Document.Selection.CharacterFormat.Subscript == FormatEffect.On)
        {
            SubscriptButton.IsChecked = true;
        }
        if (editor.Document.Selection.CharacterFormat.Subscript != FormatEffect.On)
        ...
        if (editor.Document.Selection.CharacterFormat.Superscript == FormatEffect.On)
        {
            SuperscriptButton.IsChecked = true;
        }
        if (editor.Document.Selection.CharacterFormat.Superscript != FormatEffect.On)
    ...
    {
        if (editor.Document.Selection.ParagraphFormat.LineSpacing == 1 + FSize.Value && editor.Document.Selection.ParagraphFormat.LineSpacingRule == LineSpacingRule.Multiple)
        {

        }
        if (editor.Document.Selection.ParagraphFormat.LineSpacing == 1.15 + FSize.Value && editor.Document.Selection.ParagraphFormat.LineSpacingRule == LineSpacingRule.Multiple)
        {

        }
        if (editor.Document.Selection.ParagraphFormat.LineSpacing == 1.5 + FSize.Value && editor.Document.Selection.ParagraphFormat.LineSpacingRule == LineSpacingRule.Multiple)
        {

        }
        if (editor.Document.Selection.ParagraphFormat.LineSpacing == 2 + FSize.Value && editor.Document.Selection.ParagraphFormat.LineSpacingRule == LineSpacingRule.Multiple)
        {
    ...
    private void Findeditor_TextChanged(object sender, RoutedEventArgs e)
    {

    }

    private void Findeditor_KeyDown(object sender, KeyRoutedEventArgs e)
    {
        if (e.Key == VirtualKey.Enter)
        {
            Findeditor.IsReadOnly = false;
            editor.Document.Selection.SetRange(Findeditor.Document.Selection.StartPosition, editor.Document.Selection.EndPosition);
    ...
    {
        editor.Width = editorPanel.Width;
